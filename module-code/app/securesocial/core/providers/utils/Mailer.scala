...
import securesocial.core.{Identity, BasicProfile}
...
trait Mailer {
  ...
  def sendNotificationEmail(email: String, user: BasicProfile, request: Request[AnyContent])
}
...
object Mailer {
  ...
  class Default(mailTemplates: MailTemplates) extends Mailer {
    ...
    override def sendNotificationEmail(email: String, user: BasicProfile, request: Request[AnyContent]) {
